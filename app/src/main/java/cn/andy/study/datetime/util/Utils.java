package cn.andy.study.datetime.util;
...
public class Utils {
    ...
    //获取带透明度的圆角矩形
    public static Drawable getRoundDrawable(int alpha, int color, int radir) {
        int normalColor = Color.argb(alpha, Color.red(color), Color.green(color), Color.blue(color));
        Drawable normalDrawable = getRoundDrawable(normalColor, radir);
        return normalDrawable;
    }


    //根据颜色获取圆角矩形
    public static Drawable getRoundDrawable(int color, int radir) {
        radir = px(radir);
